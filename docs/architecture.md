...
On `init` the host starts a `tsnet.Server` in a state directory of its own under the user's config directory (`~/Library/Application Support/tailtab/<profile-uuid>/` on macOS, `~/.config/tailtab/` on Linux, `%AppData%\tailtab\` on Windows), and watches Tailscale's IPN bus. Every change becomes a status event: node state, login URL, tailnet, device name and address, exit nodes, accounts, peers, health warnings, the proxy port and the proxy credential. The extension renders only what the host reports.
